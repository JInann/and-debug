...
static instance(){if(!PerfUI.LineLevelProfile.Performance._instance)
PerfUI.LineLevelProfile.Performance._instance=new PerfUI.LineLevelProfile.Performance();return PerfUI.LineLevelProfile.Performance._instance;}};PerfUI.LineLevelProfile.Memory=class{constructor(){this._helper=new PerfUI.LineLevelProfile._Helper('memory');}
reset(){this._helper.reset();}
...
return;const line=node.callFrame.lineNumber+1;helper.addLineData(target,script,line,node.selfSize);}}
static instance(){if(!PerfUI.LineLevelProfile.Memory._instance)
PerfUI.LineLevelProfile.Memory._instance=new PerfUI.LineLevelProfile.Memory();return PerfUI.LineLevelProfile.Memory._instance;}};PerfUI.LineLevelProfile._Helper=class{constructor(type){this._type=type;this._locationPool=new Bindings.LiveLocationPool();this._updateTimer=null;this.reset();}
...
_createElement(type,value){const element=createElementWithClass('div','text-editor-line-marker-text');if(type==='performance'){const intensity=Number.constrain(Math.log10(1+10*value)/5,0.02,1);element.textContent=Common.UIString('%.1f',value);element.style.backgroundColor=`hsla(44, 100%, 50%, ${intensity.toFixed(3)})`;element.createChild('span','line-marker-units').textContent=ls`ms`;}else{const intensity=Number.constrain(Math.log10(1+2e-3*value)/5,0.02,1);element.textContent=Common.UIString('%.0f',value/1024);element.style.backgroundColor=`hsla(217, 100%, 70%, ${intensity.toFixed(3)})`;element.createChild('span','line-marker-units').textContent=ls`KB`;}
return element;}};;PerfUI.LiveHeapProfile=class{run(){SDK.targetManager.observeModels(SDK.HeapProfilerModel,this);requestIdleCallback(()=>this.onUpdateProfiles());PerfUI.LiveHeapProfile.hasStartedForTest(true);}
static hasStartedForTest(started){if(!PerfUI.LiveHeapProfile._startedPromise)
PerfUI.LiveHeapProfile._startedPromise=new Promise(r=>PerfUI.LiveHeapProfile._startedCallback=r);if(started)
PerfUI.LiveHeapProfile._startedCallback();return PerfUI.LiveHeapProfile._startedPromise;}
modelAdded(model){model.startSampling(1024);}
modelRemoved(model){model.stopSampling();}
async onUpdateProfiles(){const models=SDK.targetManager.models(SDK.HeapProfilerModel);const profiles=await Promise.all(models.map(model=>model.getSamplingProfile()));const lineLevelProfile=PerfUI.LineLevelProfile.Memory.instance();lineLevelProfile.reset();for(let i=0;i<profiles.length;++i){if(profiles[i])
lineLevelProfile.appendHeapProfile(profiles[i],models[i].target());}
const updateInterval=Host.isUnderTest()?10:5000;setTimeout(()=>requestIdleCallback(()=>this.onUpdateProfiles()),updateInterval);}};;PerfUI.uiLabelForNetworkPriority=function(priority){return PerfUI._priorityUILabelMap().get(priority)||'';};PerfUI.uiLabelToNetworkPriority=function(priorityLabel){if(!PerfUI._uiLabelToPriorityMapInstance){PerfUI._uiLabelToPriorityMapInstance=new Map();PerfUI._priorityUILabelMap().forEach((value,key)=>PerfUI._uiLabelToPriorityMapInstance.set(value,key));}
...
//# sourceURL=https://chrome-devtools-frontend.appspot.com/serve_rev/@015deec36d00c1f36b96fed01b8e913faf2b1e6c/perf_ui/perf_ui_module.js